...
export const bulkApi = {
  upload: async (files: File[]): Promise<{ jobId: string; status: string }> => {
    // Try presigned S3 upload first, fallback to direct multipart upload
    try {
      return await presignedUpload(files);
    } catch {
      return await directUpload(files);
    }
  },
...
async function directUpload(files: File[]): Promise<{ jobId: string; status: string }> {
  const formData = new FormData();
  files.forEach(f => formData.append('files', f));
  const res = await fetch(buildUrl('/upload'), { method: 'POST', body: formData });
  if (!res.ok) {
    const body = await res.text().catch(() => '');
    throw new ApiError(body || 'Direct upload failed', res.status);
  }
  const { data } = await res.json() as ApiEnvelope<{ jobId: string; status: string }>;
  return data;
}

async function presignedUpload(files: File[]): Promise<{ jobId: string; status: string }> {
